...
complexType complexType::operator-(const complexType& otherComplex) const
{
    return complexType(realPart - otherComplex.realPart, imaginaryPart - otherComplex.imaginaryPart);
}

// overload the oeprator /
complexType complexType::operator/(const complexType& otherComplex) const
{
    double denominator = otherComplex.realPart * otherComplex.realPart + otherComplex.imaginaryPart * otherComplex.imaginaryPart;
    if (denominator == 0) {
        cout << "Attempted to divide by zero" << endl;
        return *this;
    }
    return complexType((realPart * otherComplex.realPart + imaginaryPart * otherComplex.imaginaryPart) / denominator,
                       (imaginaryPart * otherComplex.realPart - realPart * otherComplex.imaginaryPart) / denominator);
}
